...
export default function SettingsRoot() {
  ...
  const handleWipeData = () => {
    showToast('This action requires confirmation on web', 'error');
  };
    ...
    <div className="p-4 pb-8 space-y-5">
      <div className="liquid-glass rounded-2xl p-4 flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="text-sm font-bold text-white truncate">Pranav Suresh</h4>
          <p className="text-[11px] text-[#8D99AE] font-mono-tech">{studentProfile.admissionNo}</p>
          <p className="text-[10px] text-[#00F5D4]">{studentProfile.course} — Batch {studentProfile.batch}</p>
        </div>
      </div>
