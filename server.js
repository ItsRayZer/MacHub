...
const fs       = require('fs');
const { compileRankings } = require('./tools/rankings_compiler');
const { scrapeAndUpdateDatabase, startMonthlyScrapeScheduler } = require('./tools/website_scraper');
...
/** Trigger manual compiled rankings sync */
app.post('/api/rankings/update', rateLimitMiddleware, async (req, res) => {
  console.log('[Rankings API] Manual trigger received');
  try {
    const rankings = await compileRankings();
    res.json({ success: true, count: rankings.length, message: 'Rankings updated successfully.' });
  } catch (err) {
    console.error('[Rankings API] Compilation failed:', err.message);
...
/**
 * Trigger Scraper manual run
 * GET /api/admin/trigger-scrape
 */
app.get('/api/admin/trigger-scrape', async (req, res) => {
  try {
    console.log('[Scraper Route] Manual trigger requested...');
    const result = await scrapeAndUpdateDatabase();
...
app.listen(PORT, () => {
  ...
  // ─── Rankings Scheduler (BCA 2025) ────────────────────────────────────────
  setTimeout(() => {
    console.log('[Rankings Scheduler] Running startup compilation...');
    compileRankings().catch(err => console.error('[Rankings Scheduler] Startup compile failed:', err.message));
  }, 5000);

  setInterval(() => {
    console.log('[Rankings Scheduler] Running scheduled 24h compilation...');
    compileRankings().catch(err => console.error('[Rankings Scheduler] Daily compile failed:', err.message));
  }, 24 * 60 * 60 * 1000);

  // ─── Scraper Scheduler (Monthly College Scrape) ──────────────────────────
