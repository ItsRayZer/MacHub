...
function parseProfile(html) {
  ...
  if (!result.name) {
    const possibleNameElements = [
      $full('span[id*="Name" i]'),
      $full('span[id*="Student" i]'),
      $full('span[id*="FullName" i]'),
      $full('td[id*="Name" i]'),
      $full('.student-name'),
      $full('.profile-name')
    ];
    for (const el of possibleNameElements) {
      if (el.length) {
        ...
        if (txt && txt.length > 2 && !/^(student|name|profile|admission|course|batch|dob|phone|email)$/i.test(txt)) {
          result.name = txt;
          break;
        }
      }
  ...
  if (!result.name) {
    const matches = [
      bodyText.match(/Name of Student\s*[:\s-]+\s*([A-Z\s]+?)(?:\s{2,}|\s*[A-Z][a-z]|\s*Admn|\s*Course)/i),
      bodyText.match(/Student Name\s*[:\s-]+\s*([A-Z\s]+?)(?:\s{2,}|\s*[A-Z][a-z]|\s*Admn|\s*Course)/i),
      bodyText.match(/Name\s*[:\s-]+\s*([A-Z\s]+?)(?:\s{2,}|\s*[A-Z][a-z]|\s*Admn|\s*Course)/i),
      bodyText.match(/Name of Candidate\s*[:\s-]+\s*([A-Z\s]+?)(?:\s{2,}|\s*[A-Z][a-z]|\s*Admn|\s*Course)/i)
    ];
  ...
  // Robust Admission Number fallbacks
  if (!result.admissionNo) {
    const possibleAdmElements = [
      $full('span[id*="Admission" i]'),
      $full('span[id*="Adm" i]'),
      $full('span[id*="Roll" i]'),
      $full('span[id*="No" i]')
    ];
    for (const el of possibleAdmElements) {
      if (el.length) {
        const txt = el.text().trim();
        if (txt && /^\d{4,6}$/.test(txt)) {
          result.admissionNo = txt;
          break;
        }
      }
    }
  ...
  if (!result.admissionNo) {
    const matches = [
      bodyText.match(/Admission No\s*[:\s-]+\s*(\d+)/i),
      bodyText.match(/Adm No\s*[:\s-]+\s*(\d+)/i),
      bodyText.match(/Admission Number\s*[:\s-]+\s*(\d+)/i),
      bodyText.match(/Admn\.?\s*No\s*[:\s-]+\s*(\d+)/i)
    ];
    for (const m of matches) {
      if (m) {
        result.admissionNo = m[1].trim();
        break;
      }
    }
  }
  ...
  if (!result.course) {
    const matches = [
      bodyText.match(/Course\s*[:\s-]+\s*([A-Z\s()&,-]+?)(?:\s{2,}|\s*[A-Z][a-z]|\s*Batch)/i),
      bodyText.match(/Programme\s*[:\s-]+\s*([A-Z\s()&,-]+?)(?:\s{2,}|\s*[A-Z][a-z]|\s*Batch)/i),
      bodyText.match(/Department\s*[:\s-]+\s*([A-Z\s()&,-]+?)(?:\s{2,}|\s*[A-Z][a-z]|\s*Batch)/i)
    ];
    for (const m of matches) {
      if (m) {
  ...
  if (!result.batch) {
    const matches = [
      bodyText.match(/Batch\s*[:\s-]+\s*(\d{4}\s*-\s*\d{4})/i),
      bodyText.match(/Period of Study\s*[:\s-]+\s*(\d{4}\s*-\s*\d{4})/i),
      bodyText.match(/Duration\s*[:\s-]+\s*(\d{4}\s*-\s*\d{4})/i)
    ];
    for (const m of matches) {
      if (m) {
        result.batch = m[1].trim();
        break;
      }
    }
  }
